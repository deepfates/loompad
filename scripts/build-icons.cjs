...
try {
  ...
  sizes.forEach(size => {
    try {
      execSync(`sharp -i ${path.join(assetsDir, `icon-${size}.svg`)} -o ${path.join(assetsDir, `icon-${size}.png`)} resize ${size} ${size}`, { cwd: assetsDir });
  ...
  try {
    execSync(`sharp -i ${path.join(assetsDir, 'icon-512-maskable.svg')} -o ${path.join(assetsDir, 'icon-512-maskable.png')} resize 512 512`, { cwd: assetsDir });
