    ...
    const stream = await openai.completions.create(
      {
        ...
        // Intentionally do not pass 'stop' sequences to the upstream API (we omit the 'stop' parameter entirely).
        // Rationale: Passing stop sequences to the API may cause the model to truncate or omit delimiters,
        // which can result in incomplete or altered output. Instead, we use server-side boundary detection
        // (see boundaryRegex above) to semantically identify stopping points in the streamed response.
        // This approach ensures that delimiters are preserved exactly as generated by the model,
        // and allows for more flexible or context-aware stopping compared to standard stop sequence handling.
        stream: true,
      },
